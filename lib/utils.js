...
/** Elige el enlace más relevante para "ver" un proyecto (sistema > demo > landing > primero). */
export function primaryProjectLink(links) {
  if (!links?.length) return null;
  return (
    links.find((l) => l.type === "SISTEMA") ||
    links.find((l) => l.type === "DEMO") ||
    links.find((l) => l.type === "LANDING_PAGE") ||
    links[0]
  );
}
