 ...
 */
export async function saveFile(buffer, originalName, mimeType, options = {}) {
  ...
  if (process.env.VERCEL) {
    // En Vercel el filesystem es de solo lectura / efímero fuera de /tmp: un
    // archivo "guardado" aquí no sobrevive a la siguiente invocación ni a un
    // redeploy. Sin BLOB_READ_WRITE_TOKEN no hay dónde persistirlo de verdad.
    throw new Error(
      "El almacenamiento local no funciona en Vercel. Conecta un Blob Store: " +
        "Vercel → tu proyecto → Storage → Create Database → Blob, y vuelve a intentar."
    );
  }
